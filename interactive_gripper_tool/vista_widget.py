...
class VistaWidget(QWidget):
    ...
    信号:
        point_picked_signal(dict): 当一个点被拾取时发射。
                                     发出的字典格式为:
                                     {'actor_name': str, 'world_coord': [x, y, z]}
    """
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # 6. 连接 PyVista 的点击事件
        #    我们使用 'track_click_position' 来获取点击的actor和坐标
        self.plotter.track_click_position(callback=self._on_click_callback, side='left')
    ...
    def _on_click_callback(self, plotter: QtInteractor) -> None:
        """
        当 PyVista 视窗被点击时由 track_click_position 调用的内部回调。
        """
        # 1. 检查拾取模式是否激活
        ...
        # 2. 获取拾取到的 actor 和点
        #    'plotter.picked_actor' 和 'plotter.picked_point' 会被自动填充
        picked_actor = plotter.picked_actor
        picked_point = plotter.picked_point

        # 3. 检查是否点到了有效目标
        if picked_actor is None or picked_point is None:
            print("VistaWidget: 点击了空白区域。")
            return
        ...
        if actor_name is None:
            # (这可能点到的是坐标轴或我们创建的临时'拾取球'，忽略即可)
            print(f"VistaWidget: 拾取到一个未被管理的 actor (例如: 坐标轴)。")
            return
        ...
        pick_data = {
            ...
            'world_coord': list(picked_point) # 转为 list 以便 Qt 信号处理
        }
...
if __name__ == '__main__':
    ...
    def on_pick(data):
        ...
        print(f"坐标: {data['world_coord']}")
        label.setText(f"拾取到: {data['actor_name']} @ {np.array(data['world_coord']).round(2)}")
