...
```text
scripts/
├── main.py                 # 入口 (解析 --load-default 等)
├── main_window.py          # 主窗口 (委托 ui_builder + wiring)
├── ui/                     # ui_builder.py / wiring.py / anchor_view.py
├── vista_widget.py         # 3D 视图组件
├── controls_widget.py      # 控件面板
├── data_manager.py         # 数据/状态中枢 (加载资产, 发射信号)
├── anchor_manager.py       # 锚点拾取与 CRUD 抽离
├── optimization_thread.py  # 后端优化 + FK (JAX/pyroki)
└── optimization/           # energy_functions / optimizers / optimizer_state
```
...
- 功能：
    ...
    -- UI 构建由 `ui/ui_builder.py` 负责；手/锚点帧更新由 `ui/anchor_view.py` 封装。
    -- 信号集中在 `ui/wiring.py` 的 `connect_all(MainWindow, AnchorViewCoordinator)` 中，减少主类样板代码。
    -- 动态手位姿更新信号：`optimization_thread.pose_update_signal -> anchors.on_pose_update_with_anchors`。
...
-- 主要属性（精简后）：
    - `self.object_mesh`, `self.hand_links_mesh_dict`
    - `self.anchor_manager`（包含 `anchor_pairs`、拾取模式与临时点）
    - `self.current_link_poses`（用于锚点跟随动态手）
    - `self.selected_hand_name`（用于 hand_config 选择）
...
- 功能要点：
    ...
    -- 槽：
        - `trigger_optimization(anchor_pairs: list)`：更新当前锚点集并继续迭代
        - `set_visual_link_names([...])`：限制输出 link，避免缺失 actor 警告

-- 信号：
    - `pose_update_signal = pyqtSignal(dict)`：发射 `{dyn_hand_<link>: pose}`；在 `AnchorViewCoordinator` 中统一更新视图与锚点位置。
